...
from pipeline.clinical_recommendations import generate_recommendations
...
async def analyze_report(
    ...
    # Generate Clinical Recommendations based on extracted entities
    recommendations = await asyncio.to_thread(generate_recommendations, merged_entities)

    # ROUGE scores — Compute only if a reference summary is provided
    rouge = None
    if reference_summary and len(reference_summary.strip()) > 10:
        try:
            rouge = await asyncio.to_thread(compute_rouge_scores, reference_summary.strip(), summary)
        except Exception:
            pass
...
SYNTHETIC_DOCS = [
    {
        ...
        "reference_summary": "52-year-old male with poorly controlled Type 2 Diabetes Mellitus presenting with fatigue, polyuria, and polydipsia. Fasting glucose is elevated. Plan is to increase Metformin to ER, add Sitagliptin, and order eye and foot exams."
    },
    {
        ...
        "reference_summary": "68-year-old female admitted for acute decompensated CHF, AFib, and Diabetes. Successfully diuresed with improved symptoms. Transitioned to oral diuretics and started on Carvedilol and Warfarin. Discharged with strict low-sodium diet and fluid restriction instructions."
    },
    {
        ...
        "reference_summary": "61-year-old male with Stage IV NSCLC on a Phase II trial (Osimertinib + Bevacizumab) showing sustained partial response at Cycle 6. Primary and metastatic lesions have significantly reduced. ctDNA reduced by 95.5%. Managing mild side effects including hypertension and paronychia. Plan to continue current regimen."
    }
]
...
async def test_synthetic_docs():
    ...
    for doc in SYNTHETIC_DOCS:
        ...
        recommendations = await asyncio.to_thread(generate_recommendations, raw_entities)

        rouge = None
        try:
            rouge = await asyncio.to_thread(compute_rouge_scores, doc["reference_summary"], summary_result["summary"])
        except Exception:
            pass
